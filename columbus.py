# The original code can be found at https://github.com/openstreetmap/svn-archive/blob/main/applications/utils/gps-tracks/columbus-to-gpx/columbus-to-gpx.py
...
def convertone(fn: str, outfn: str) -> None:
    waypoints: list[tuple[str, str, str, str]] = []
    oldlat, oldlon = None, None  # Initialize oldlat and oldlon
    with open(fn, "r") as inf, open(outfn, "w") as outf:
        ...
        for line in inf:
            ...
            if oldlat is None or oldlon is None:
                oldlat = lat
                oldlon = lon
            if fields[1] == 'V':
                audiofn = fields[9].replace("\000", "").rstrip()
                waypoints.append((oldlat, oldlon, trkptdate, f"{audiofn}.wav"))
            elif fields[1] == 'C':
